...
def evaluate(loader, agent, split, opts):
            ...
            for j in azimuths:
                ...
                if i == elev_to_vis and j == azim_to_vis:
                    ...
                    for idx in range(batch_size):
                        ...
                        _, pred_label = torch.max(predictions_all[-1][idx], dim=0)
    ...
    scores_across_time = []
    gt_labs = gt_labs.long()
    for t in range(opts.T):
        _, argmax_pred = torch.max(pred_labs_across_time[t], dim=1)
        score = accuracy_score(argmax_pred.numpy(), gt_labs.numpy())
        scores_across_time.append(score)
